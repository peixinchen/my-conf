...
"focus
exec "Snippet incd include_once $_SERVER['DOCUMENT_ROOT'] . '".st."Path".et."' ;<CR>".st.et
exec "Snippet msf mysql_query_focus($query,'".st."db".et."');<CR>".st.et
exec "Snippet mfo mysql_fetch_object($result)<CR>".st.et
...
exec "Snippet $_G $_GLOBALS['".st."variable".et."']".st.et
exec "Snippet fune if(!function_exists('".st."FunctionName".et."')){<CR>function ".st."FunctionName".et."(".st."".et."){<CR>".st."".et."<CR>}<CR>}<CR>".st.et
exec "Snippet try try{<CR>".st.et."<CR>}<CR>catch (".st."Exception".et."){<CR>".st.et."<CR>}<CR>"
exec "Snippet =$ <?=$".st."variable".et."?>".st.et
exec "Snippet e$ <?echo ".st."variable".et.";?>".st.et
exec "Snippet ifh <?php if (".st.") {?><CR>".st."<CR><?php }?><CR>".st.et
