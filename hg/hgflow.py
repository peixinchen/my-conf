...
class HgFlow(object):
    ...
    def _getPatternBranches(self, p):
        branches = []
        activebranches = [self.repo[n].branch() for n in self.repo.heads()]
        for b in activebranches:
            if re.match(p, b):
                if self.repo.branchheads(b, closed=False):
                    branches.append(b)
    ...
    def hgflow_func_init(self, args, opts):
        ...
        if not develop_branch in branches:
            ...
            commands.update(self.ui, self.repo, develop_branch)
    ...
    def hgflow_func_short(self, cmd, name, args, opts):
        ...
        if (name != 'list' and self._hasUncommit()):
            return
        ...
        if opts['list']:
              p = '^'+prefix
              branches=self._getPatternBranches(p)
              for b in branches:
                  self.outputln(b)
              return

        if name is None:
            self.outputln(_('Current branch is not '+cmd+' branch'))
            return
        ...
        if not self._findBranch(target_branch, name):
            ...
            if opts['finish']:
                ...
                commands.update(self.ui, self.repo, 'develop')
                return
            else:
                #switch
...
def hgflow_cmd(ui, repo, cmd, *args, **opts):
    for o in ['start', 'switch', 'finish', 'close', 'list']:
        _fill_opts(opts, o)
    _fill_opts(opts, 'rev', '')
...
cmdtable = {
    ...
    'feature' : (
      ...
      [ ('c', 'close',  None, 'close the branch'),
        ...
        ('r', 'rev',    '',   'The specified revision'),
        ('l', 'list',   None,   'list all feature branches'),
    ...
    'release' : (
      ...
      [ ('c', 'close',  None, 'close the branch'),
        ('s', 'switch', None, 'switch the branch'),
        ('r', 'rev',    '',   'The specified revision'),
        ('l', 'list',   None,   'list all release branches'),
        ('f', 'finish', None, 'finish the branch') ],
    ...
    'hotfix' : (
      ...
      [ ('c', 'close',  None, 'Close the branch'),
        ('s', 'switch', None, 'Switch the branch'),
        ('r', 'rev',    '',   'The specified revision'),
        ('l', 'list',   None,   'list all hotfix branches'),
        ('f', 'finish', None, 'Finish the branch') ],
